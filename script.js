...
let levels = {
    ...
    2: { topic: 'Variables', notes: 'Variables store values using let, const, or var.\n\nSyntax: let x = value;\nExample: let count = 5;', prompt: 'Declare a variable "count" with value 10.', solution: 'let count = 10;' },
    3: { topic: 'Data Types', notes: 'Common data types: string, number, boolean, null, undefined, object.\nExample: let flag = true; let text = "hello";', prompt: 'Create a boolean variable "isActive" and set it to true.', solution: 'let isActive = true;' },
    4: { topic: 'Functions', notes: 'Functions allow reusable blocks of code.\n\nSyntax: function name(params){...}\nExample: function add(a,b){ return a+b; }', prompt: 'Write a function "double" that returns a number multiplied by 2.', solution: 'function double(n){ return n*2; }' },
    5: { topic: 'Arrays', notes: 'Arrays store multiple values.\n\nSyntax: let arr = [val1, val2];\nExample: let nums = [1,2,3];', prompt: 'Create an array "colors" containing "red", "green", "blue".', solution: 'let colors = ["red","green","blue"];' },
    6: { topic: 'Objects', notes: 'Objects store key-value pairs.\n\nSyntax: let obj = {key: value};\nExample: let point = {x:1, y:2};', prompt: 'Create an object "book" with properties title="JS Guide" and pages=100.', solution: 'let book = {title:"JS Guide", pages:100};' },
    7: { topic: 'Loops', notes: 'Loops repeat code blocks: for, while, do-while.\nExample: for(let i=0;i<3;i++){ console.log(i); }', prompt: 'Write a for loop to print numbers 1 through 5.', solution: 'for(let i=1;i<=5;i++){ console.log(i); }' },
    8: { topic: 'Conditionals', notes: 'Use if, else if, else to make decisions.\nExample: if(x>10){ console.log("big"); } else { console.log("small"); }', prompt: 'Write code to check if variable "num" is even or odd and print the result.', solution: 'if(num%2===0){ console.log("even"); } else { console.log("odd"); }' },
    9: { topic: 'Events', notes: 'Events respond to user actions.\nSyntax: element.addEventListener("click", function(){...});\nExample: button.addEventListener("click", ()=>alert("Clicked"));', prompt: 'Write code to alert "Clicked!" when a button is clicked.', solution: 'document.querySelector("button").addEventListener("click", ()=>alert("Clicked!"));' },
    10: { topic: 'DOM Manipulation', notes: 'DOM allows dynamic page updates.\nSyntax: document.getElementById("id").textContent = "text";\nExample: document.getElementById("demo").textContent="Hello";', prompt: 'Change the text of an element with id "message" to "Hello World".', solution: 'document.getElementById("message").textContent = "Hello World";' }
};
